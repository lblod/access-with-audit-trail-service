...
function requestReadReasonQuery(
  ...
  const { shapeUri, shape } = buildPath(paths);
  return `
    ${PREFIXES}
    INSERT DATA {
...
function buildPath(paths = []) {
  ...
  if (listPath.length === 1) {
    return {
      ...
      shape: `
        ...
        ${shapeUri} <http://www.w3.org/ns/shacl#path> <${listPath[0]}>.`;
    }
  }
  ...
  function buildList(paths = [], acc = []) {
    ...
    acc.push(`${listUri} a <http://www.w3.org/1999/02/22-rdf-syntax-ns#List>`);
    acc.push(
